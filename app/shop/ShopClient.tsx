...
export default function ShopClient() {
  ...
  const [designFilter, setDesignFilter] = useState<DesignLine | "All">(
    DESIGN_LINES.includes(initialDesign as DesignLine) ? (initialDesign as DesignLine) : "All"
  );
  ...
  const products = filtered.flatMap((nation) => {
    const designs = designFilter === "All" ? [...DESIGN_LINES] : [designFilter];
    return designs.map((design) => ({ nation, design: design as DesignLine }));
  });
  ...
  return (
    <div className="min-h-screen pt-14">
      ...
      >
        <span className="text-label" style={{ color: "var(--color-accent)" }}>Collection</span>
        <h1 className="text-display-lg text-white mt-1">Shop All</h1>
        <p className="text-body-sm mt-2" style={{ color: "#888" }}>
          {products.length} designs · {filtered.length} nations
        </p>
      </div>
          ...
          <div className="flex gap-1.5 overflow-x-auto scrollbar-hide pb-1">
            {REGIONS.map((r) => (
              <button
                ...
                style={region === r ? { background: "var(--color-accent)" } : { background: "#161616" }}
              >
                {r}
              </button>
            ))}
      ...
      <div className="max-w-[var(--max-width)] mx-auto px-[var(--container-px)] lg:px-[var(--container-px-lg)] py-6 lg:py-10">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 lg:gap-5">
          {products.slice(0, showCount).map(({ nation, design }) => (
            <ProductCard key={`${nation.code}-${design}`} nation={nation} design={design} />
          ))}
        </div>
        {products.length > showCount && (
          <div className="text-center mt-8">
            ...
            >
              Load More ({products.length - showCount})
            </button>
          </div>
        )}
      </div>
