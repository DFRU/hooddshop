...
export default async function NationsPage() {
  ...
  // Build a map: nation code → matching Shopify product + vehicle image
  const nationProductMap: Record<
    string,
    { handle: string; imageUrl: string | null; vehicleImageUrl: string | null }
  > = {};
  for (const nation of NATIONS) {
    ...
    const vehicleImg = getHeroVehicleImage(nation.code);
    if (match) {
      nationProductMap[nation.code] = {
        handle: match.handle,
        imageUrl: match.images?.edges?.[0]?.node?.url ?? null,
        vehicleImageUrl: vehicleImg?.src ?? null,
      };
    }
  }
